...
async def apply_drip_not_before(run_id: int, spread_days: int,
    ...
    gen_per_row: спины строки размазываются по дням (плавный дрип), но ОРИГИНАЛ —
    на самый ранний день своей группы. Генерация атомарна по группе (сгенерив
    оригинал, fanout материализует ВСЕ спины разом), поэтому оригинал обязан
    «созреть» не позже любого своего спина — иначе спин, чей оригинал ещё в
    будущем-за-горизонтом, застревает несгенерированным (run 1557: 949 таких).
    Оригинал никогда не позже спинов; созревшее не откладывается на потом.
    ...
    if mode == "gen_per_row" and groups:
        # Спины строки размазываем по дням (плавный дрип ссылок на домен), НО
        # ОРИГИНАЛ — на самый РАННИЙ день группы. Генерация атомарна по группе:
        # сгенерив оригинал, fanout тут же материализует ВСЕ его спины (спины —
        # спинтакс-варианты тела оригинала, отдельно не генерятся). Значит оригинал
        # обязан «созреть» не позже любого своего спина — иначе спин застрянет
        # несгенерированным до дня оригинала. Оригинал НИКОГДА не позже спинов;
        # созревшие айтемы не откладываются на потом.
        by_day: dict[int, list[int]] = {}
        for g in groups:
            oid = g.get("original_item_id")
            spins = [s for s in (g.get("spin_item_ids") or []) if s is not None]
            d0 = random.randrange(spread_days)  # оригинал: равномерно (генерация по дням)
            if oid is not None:
                by_day.setdefault(d0, []).append(oid)
            for sid in spins:  # спин: любой день НЕ раньше оригинала (плавный дрип)
                by_day.setdefault(random.randrange(d0, spread_days), []).append(sid)
        async with WriteSession() as s:
