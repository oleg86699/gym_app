...
# Reaper осиротевших CF-браузеров — теперь БЭКАП за пулом (_BrowserPool), не
# основная защита. Пул переиспользует один браузер и сам recycle-ит его по
# ≤CF_BROWSER_RECYCLE_AFTER_S (480с)/50 контекстов. Но patchright не даёт force-kill
# по PID при browser.close(), поэтому recycle иногда оставляет брошенное дерево
# chromium — его и добивает reaper. ВАЖНО: порог 900с > времени жизни браузера пула
# (≤480с), чтобы reaper НЕ бил активный браузер пула (иначе пул терял reuse и
# релончился каждые ~4 мин). mem_limit 4g — жёсткий бэкап поверх этого.
_CHROME_MAX_AGE_S = 900
