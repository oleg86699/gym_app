...
async def _bump_site_failure(
    ...
    if new_count >= _SITE_FAILURE_DISABLE_THRESHOLD and site.is_active:
        # Per-cred guard: не выключаем домен пока у него есть подтверждённо
        # рабочий cred (cred_status='valid'). На сайтах с 1 cred это поведение
        # как раньше (если cred не valid — выключаем). На сайтах со многими
        # cred рабочий доступ держит домен — transient-фейлы других cred /
        # прокси не убивают полезный сайт раньше времени.
                ...
                async with registry.claim(site.id):
                    ...
                    for cred in valid_creds:
                        ...
                        try:
                            ...
                            async with WriteSession() as s:
                                await _bump_site_failure(s, site.id, kind="network")
                                if picked_proxy:
                        ...
                        if outcome.error == ErrorKind.XMLRPC_DISABLED:
                            ...
                            async with WriteSession() as s:
                                ...
                                await _bump_site_failure(s, site.id, kind="xmlrpc_disabled")
                            registry.mark_exhausted(site.id)
                            site_done = True
                            break
                        ...
                        if outcome.error == ErrorKind.SITE_NOT_FOUND:
                            async with WriteSession() as s:
                                await _reset_site_url(s, site.id)
                                await _bump_site_failure(s, site.id, kind="site_not_found")
                            registry.mark_exhausted(site.id)
                            site_done = True
                            break
                        ...
                        if outcome.error in (
                            ...
                            async with WriteSession() as s:
                                ...
                                await _bump_site_failure(s, site.id, kind=outcome.error.value)
    ...
    try:
        ...
        async with httpx.AsyncClient(
            ...
            while True:
                ...
                # Берём батч pending text_items (только с готовым текстом —
                # пустые gen-айтемы пропускаем, их сгенерят отдельно)
                async with WriteSession() as s:
                    batch = await _pick_pending_batch(s, run_id, BATCH_SIZE, require_text=True)

                if not batch:
                    # Pending кончился. Проверим есть ли ещё «posting» в работе —
                    # маловероятно (этот же loop их забрал), но всё равно ждём
                    # подгруза, если ничего не происходит — выходим.
                    async with WriteSession() as s:
                    ...
                    # тексты» — внешняя генерация ещё наполняет тексты. Ждём, пока
                    # gen_active снимется (генерация закончилась). Stall-кап ~10 мин
                    # без прогресса (на случай SIGKILL генератора) — чтобы не виснуть.
                    ...
                    # ставим needs_review; после дозаполнения юзером run пере-дёрнут.
                    async with WriteSession() as s:
                        nr = await s.scalar(
                            select(func.count(TextItem.id)).where(
                                TextItem.posting_run_id == run_id,
                ...
                # Перед массовым запуском проверим что вообще есть сайты —
                # если нет, ставим need_more_admins и возвращаем batch обратно.
                _f_langs, _f_tlds = _run_site_filter(run)
                async with WriteSession() as s:
                    any_site = await _pick_candidate_sites(
                        s, project_id=run.project_id, run_id=run.id, exclude_site_ids=set(), limit=1,
                        site_langs=_f_langs, site_tlds=_f_tlds,
                    )
                if not any_site:
                    # Вернуть pending для будущих попыток (если юзер добавит сайтов)
                    async with WriteSession() as s:
                        await s.execute(
                            update(TextItem)
                            .where(
                ...
                before_posted = await _read_posted_count(run_id)
                # return_exceptions=True ВАЖНО: один сбой httpx (dead proxy,
                # disconnect, и т.п.) НЕ должен валить весь run. Логируем
                # каждое исключение и продолжаем с остальными items.
                ...
                for item, res in zip(batch, results, strict=True):
                    if isinstance(res, BaseException):
                        log_ctx.exception(
                            "posting.item.unexpected",
                            item_id=item.id, error=str(res),
                        )
                        async with WriteSession() as s:
                            await _mark_text_failed(
                                s, item_id=item.id,
                                error_message=f"worker exception: {str(res)[:200]}",
                            )
                            await _bump_run_counter(s, run.id, "failed_count")
                after_posted = await _read_posted_count(run_id)

                # Если registry полностью «исчерпан» (все доступные сайты упали в
                # exhausted) и за раунд не было прогресса — выходим в
                # need_more_admins.
                if after_posted == before_posted:
                    no_progress_rounds += 1
                    if no_progress_rounds >= 2:
                        # Проверим: может правда сайтов больше нет?
                        _f_langs, _f_tlds = _run_site_filter(run)
                        async with WriteSession() as s:
                            any_site = await _pick_candidate_sites(
                                s,
                                project_id=run.project_id,
                                run_id=run.id,
                                exclude_site_ids=set(),  # без in-memory exclude
                                limit=1,
                                site_langs=_f_langs, site_tlds=_f_tlds,
                            )
                        if not any_site:
                            ...
                            break
                        # сайты есть — обнуляем in-memory exhausted (мог быть
                        # transient network blip) и пробуем снова
